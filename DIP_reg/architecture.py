...
def get_decoder(x, unet_connections):
    ...
    layer_index = len(layer_depth) - parameters.paramatarisation_layer

    if layer_index > 0:
        layer_layers = layer_layers[:-layer_index]
        layer_depth = layer_depth[:-layer_index]
        layer_groups = layer_groups[:-layer_index]
    ...
    x = layers.get_convolution_layer(x, 3, (3, 3, 3), (1, 1, 1), 1, True, "output")
...
def get_tensors(input_shape):
    ...
    latent_layers.reverse()

    layer_index = len(latent_layers) - parameters.paramatarisation_layer

    if layer_index > 0:
        latent_layers = latent_layers[:-layer_index]
